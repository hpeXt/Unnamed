...
impl DependencyResolver {
    ...
    /// 检查依赖是否满足
    pub fn check_dependencies_satisfied(&self, plugin_name: &str, available_plugins: &[String]) -> bool {
        if let Some(deps) = self.dependencies.get(plugin_name) {
    ...
    pub fn get_stats(&self) -> (usize, usize, usize) {
        ...
        let plugins_with_deps = self.dependencies.values().filter(|deps| !deps.is_empty()).count();
...
mod tests {
    ...
    fn test_multiple_plugins() {
        ...
        let order = resolver.resolve_order(&["plugin1".to_string(), "plugin2".to_string()]).unwrap();
