...
use super::host_functions::{HostContext, create_context_store, build_plugin_with_host_functions};
use super::manifest::{PluginManifest, find_and_read_manifest};
...
impl PluginLoader {
    /// 创建新的插件加载器
    pub fn new(msg_sender: mpsc::Sender<Message>, storage: Arc<Storage>, identity: Option<Arc<IdentityManager>>) -> Result<Self> {
    ...
    /// 调用插件函数
    pub fn call_plugin<I, O>(&mut self, plugin_name: &str, function_name: &str, input: I) -> Result<O>
    where
        I: serde::Serialize,
        O: serde::de::DeserializeOwned,
    {
        ...
        // 调用插件函数
        let output = plugin.call::<&str, &str>(function_name, &input_json)
    ...
    /// 调用插件函数（返回字符串）
    pub fn call_plugin_string(&mut self, plugin_name: &str, function_name: &str, input: &str) -> Result<String> {
        let plugin = self.get_plugin_mut(plugin_name)?;
    ...
    pub fn scan_and_load_plugins(&mut self, plugin_dir: &Path) -> Result<Vec<String>> {
            ...
            .filter(|e| e.path().extension().map_or(false, |ext| ext == "wasm"))
        {
            ...
            match self.load_plugin(&plugin_name, wasm_path.to_str().unwrap()) {
                ...
                Err(e) => {
                    tracing::warn!("加载插件失败: {} ({}): {}", plugin_name, wasm_path.display(), e);
                }
            }
        }
    ...
    /// 按配置加载插件
    pub fn load_plugins_from_config(&mut self, plugin_dir: &Path, enabled_plugins: &[String]) -> Result<Vec<String>> {
        let mut loaded_plugins = Vec::new();
        ...
        for plugin_name in enabled_plugins {
            ...
            let possible_paths = vec![
                ...
                plugin_dir.join(plugin_name).join(format!("{}.wasm", plugin_name)),
                plugin_dir.join(plugin_name).join("target/wasm32-unknown-unknown/release").join(format!("{}.wasm", plugin_name)),
            ];
            ...
            for path in possible_paths {
                if path.exists() {
                    match self.load_plugin(plugin_name, path.to_str().unwrap()) {
                        ...
                        Err(e) => {
                            tracing::warn!("加载插件失败: {} ({}): {}", plugin_name, path.display(), e);
                        }
                    }
                }
            }
    ...
    /// 使用依赖解析加载插件
    pub fn load_plugins_with_dependencies(&mut self, plugin_dir: &Path, target_plugins: &[String]) -> Result<Vec<String>> {
        // 发现所有插件
        ...
        for plugin_name in load_order {
            ...
            if let Some(plugin_path) = self.find_plugin_path(plugin_dir, &plugin_name)? {
                match self.load_plugin(&plugin_name, plugin_path.to_str().unwrap()) {
                    ...
                    Err(e) => {
                        tracing::warn!("依赖加载失败: {} ({}): {}", plugin_name, plugin_path.display(), e);
                    }
                }
            } else {
    ...
    pub fn check_dependencies(&self, plugin_name: &str) -> bool {
        ...
        self.dependency_resolver.check_dependencies_satisfied(plugin_name, &available_plugins)
    }
