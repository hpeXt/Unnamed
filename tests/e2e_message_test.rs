...
use minimal_kernel::kernel::{
    message_bus::{MessageBusHandle, MessageRouter, create_message_bus},
...
/// 创建测试环境
async fn setup_test_env() -> anyhow::Result<(PluginLoader, MessageBusHandle, MessageRouter, Arc<Storage>, TempDir)> {
    // 创建临时目录用于存储
...
async fn test_basic_message_passing() -> anyhow::Result<()> {
    ...
    let send_result: String = loader.call_plugin(
        "test_sender",
        "send_test_message",
...
async fn test_send_to_nonexistent_plugin() -> anyhow::Result<()> {
    ...
    let result: String = loader.call_plugin(
        "test_sender",
        "send_to_nonexistent",
        test_id.to_string()
    )?;
