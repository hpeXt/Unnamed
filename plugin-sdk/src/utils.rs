    ...
    impl ConfigExtractor {
        ...
        pub fn get_string(&self, key: &str) -> PluginResult<String> {
            match self.data.get(key) {
                ...
                None => Err(PluginError::Configuration(format!(
                    "Missing config key: {}",
                    key
                ))),
            }
            ...
            T: for<'de> Deserialize<'de>,
        {
            match self.data.get(key) {
                Some(v) => serde_json::from_value(v.clone()).map_err(|e| {
                    PluginError::Configuration(format!("Invalid number config '{}': {}", key, e))
                }),
                None => Err(PluginError::Configuration(format!(
                    "Missing config key: {}",
                    key
        ...
        pub fn get_bool(&self, key: &str) -> PluginResult<bool> {
            match self.data.get(key) {
                ...
                Some(v) => Err(PluginError::Configuration(format!(
                    "Invalid boolean config '{}': {}",
                    key, v
                ))),
                None => Err(PluginError::Configuration(format!(
    ...
    impl HealthCheckBuilder {
        ...
        pub fn run(self) -> HealthCheck {
            ...
            for (i, check) in self.checks.iter().enumerate() {
                match check() {
                    Ok(result) => {
                        overall.details.insert(
                            format!("check_{}", i),
                            serde_json::to_value(result.clone()).unwrap_or_default(),
                        );
                        if !result.healthy {
                            all_healthy = false;
                        }
