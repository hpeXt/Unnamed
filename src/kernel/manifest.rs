...
pub fn find_and_read_manifest(wasm_path: &Path) -> Result<PluginManifest> {
    let mut search_paths = vec![
        // 同目录
        wasm_path.parent().unwrap_or(wasm_path).join("manifest.toml"),
    ];
    ...
    // 插件目录根
    if let Some(root) = wasm_path.parent().and_then(|p| p.parent()).and_then(|p| p.parent()) {
    ...
    let plugin_name = wasm_path.file_stem()
