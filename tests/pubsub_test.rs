...
async fn test_topic_subscription() {
    ...
    // 验证两个插件都收到了消息
    let msg1 = timeout(Duration::from_secs(1), plugin1_rx.recv()).await
        .expect("插件1应该收到消息")
        .expect("消息不应该为空");
    
    let msg2 = timeout(Duration::from_secs(1), plugin2_rx.recv()).await
        .expect("插件2应该收到消息")
        .expect("消息不应该为空");
...
async fn test_multiple_topics() {
    ...
    let received1 = timeout(Duration::from_secs(1), plugin1_rx.recv()).await
        .expect("插件1应该收到消息")
        .expect("消息不应该为空");
    ...
    let received2 = timeout(Duration::from_secs(1), plugin2_rx.recv()).await
        .expect("插件2应该收到消息")
        .expect("消息不应该为空");
