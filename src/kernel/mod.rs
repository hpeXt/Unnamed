...
use message_bus::{MessageBusHandle, MessageRouter, create_message_bus};
...
impl Kernel {
    pub async fn new(config: Config) -> Result<Self> {
        ...
        let mut plugin_loader = PluginLoader::new(msg_sender, storage.clone(), Some(identity.clone()))?;
        ...
        if config.plugins.auto_load {
            ...
            let loaded_plugins = plugin_loader.load_plugins_from_config(
                &config.plugins.directory,
                &config.plugins.enabled
            )?;
            tracing::info!("已自动加载 {} 个插件: {:?}", loaded_plugins.len(), loaded_plugins);
        }
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        // 取出消息路由器
        let router = self.message_router.take()
    ...
    {
        self.plugin_loader.call_plugin(plugin_name, function_name, input)
    }
    
    /// 调用插件函数（字符串版本）
    pub fn call_plugin_string(&mut self, plugin_name: &str, function_name: &str, input: &str) -> Result<String> {
        self.plugin_loader.call_plugin_string(plugin_name, function_name, input)
    }
