...
impl Config {
    ...
    fn validate(&self) -> Result<()> {
        ...
        if !self.plugins.directory.exists() {
            tracing::warn!("插件目录不存在，将自动创建: {}", self.plugins.directory.display());
    ...
    pub fn init_logging(&self) -> Result<()> {
        ...
        match self.logging.format {
            LogFormat::Compact => {
                ...
                if let Some(log_dir) = &self.logging.directory {
                    std::fs::create_dir_all(log_dir)?;
                    let file_appender = tracing_appender::rolling::daily(log_dir, "minimal-kernel.log");
                    let (non_blocking, _guard) = tracing_appender::non_blocking(file_appender);
                    let file_layer = fmt::layer().compact().with_ansi(false).with_writer(non_blocking);
