...
use anyhow::Result;
use minimal_kernel::config::{
    Config, DatabaseConfig, IdentityConfig, LogLevel, LoggingConfig, NetworkConfig, PluginConfig,
};
...
async fn test_config_file_operations() -> Result<()> {
    ...
    assert_eq!(
        loaded_config.plugins.directory,
        original_config.plugins.directory
    );
