            ...
            PluginEvent::Message(msg) => {
                if self.fail_on_message {
                    return Err(PluginError::MessageProcessing(
                        "Mock message processing failure".to_string(),
                    ));
                }
...
impl TestAssertions {
    ...
    pub fn assert_storage_not_contains_key(storage: &MockStorage, plugin_id: &str, key: &str) {
        ...
        assert!(
            !keys.contains(&key.to_string()),
            "Storage should not contain key '{}'",
            key
        );
    }
...
macro_rules! test_plugin_lifecycle {
    ($plugin:expr, $config:expr) => {
        ...
        assert_eq!(
            $plugin.status(),
            $crate::plugin::PluginStatus::Uninitialized
        );
...
mod tests {
    ...
    fn test_mock_plugin() {
        ...
        let config = TestConfigBuilder::new().add_string("key", "value").build();
        ...
        assert_eq!(
            plugin.received_messages()[0].payload_string().unwrap(),
            "hello"
        );
    }
