...
async fn test_plugin_signing() -> Result<()> {
    ...
    let is_valid = identity
        .verify_plugin_signature(plugin_id, message, &signature)
        .await?;
    assert!(is_valid);
    ...
    let is_invalid = identity
        .verify_plugin_signature(plugin_id, wrong_message, &signature)
        .await?;
    assert!(!is_invalid);
...
async fn test_from_private_key() -> Result<()> {
    ...
    // 验证相同私钥生成相同地址
    assert_eq!(
        identity1.get_master_address(),
        identity2.get_master_address()
    );
